...
from output.receipt import build_receipt
from output.state import RecoveryBlocked, blob_sha
from test_state import MemoryState, fixture
...
class ResultReceiptTests(unittest.TestCase):
    def setUp(self):
        ...
        workflow_patch = patch(
            "output.receipt.workflow_identity",
            return_value={
    ...
    def test_failed_receipt_commit_then_rerun_preserves_one_immutable_receipt(self):
        from output import receipt as finalize_receipt
        from output.state import encoded_json, receipt_path, record_path
        ...
        with (
            patch("output.receipt.GitHubState", return_value=state),
            patch("output.receipt.identity_for", return_value=identity),
            patch("output.receipt.OUTPUT_DIR", output),
