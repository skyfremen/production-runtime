...
def bootstrap(batch_id, source_sha, output):
    ...
    print(f"Private input fetch PASS: items={len(local_requests)}")
...
def persist_registry(manifest_path):
    ...
    if current == raw:
        print("Private registry unchanged")
        return
    ...
    print("Private registry write-back PASS")
...
def complete(manifest_path, summary_path):
    ...
    print("Private completion write-back PASS")
...
if __name__ == "__main__":
    try:
        main()
    except (TransportError, KeyError, OSError, ValueError, json.JSONDecodeError) as exc:
        raise SystemExit(f"Private state boundary failed: {exc}") from None
