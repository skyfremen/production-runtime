"""V1 prepare/upload stages. Public publication is immediate and has no scheduling branch."""
...
def prepare(path):
    request=load_json(path); identity=identity_for(path,request); state=GitHubState()
    decision=prepare_upload(state,path,request,identity,make_client())
    atomic_write_json(OUTPUT_DIR/"prepare.json",{"identity":identity,**decision})
    emit("upload_required","true" if decision["upload_required"] else "false")
    print("Upload prepare PASS")
def upload(path):
    request=load_json(path); identity=identity_for(path,request); state=GitHubState()
    ...
    evidence=upload_new(state,path,request,identity,video,make_client())
    atomic_write_json(OUTPUT_DIR/"upload_result.json",{"content_id":identity["content_id"],"youtube_video_id":evidence["youtube_video_id"],"upload_evidence":evidence})
    print(f"Upload PASS {evidence['youtube_video_id']}")
def main():
