...
class ProductionPipeline:
    ...
    def prepare(self, index, request):
        ...
        self.run_command(
            ["python", "runtime/validation/validate_content.py", "--request", request],
            env,
            log_path,
        )
    ...
    def generate(self, item):
        ...
        try:
            ...
            stages["media"] = self.run_command(
                ["python", "runtime/media/media_resolver.py", "--request", item.request],
                item.env,
                log_path,
            )
            stages["render"] = self.run_command(
                ["python", "runtime/rendering/render_aligned.py", "--request", item.request],
                item.env,
                log_path,
            )
            stages["verify"] = self.run_command(
                ["python", "runtime/rendering/verify_render.py", "--request", item.request],
                item.env,
                log_path,
            )
